...
# option for two percentage stack plots at present to make percentages of 
...
# but as an example the example plots in the manual take about 35 seconds from
...
# the code should run fine in mac or linux especially in an IDE. It may need 
# some adjustment based on how linux/mac deal with the cmd line.
...
print("School of Geography, University of Leeds, 2021")

# The program uses a user preferences form the Parameter file, the location of
# which is supplied in the information above from the cmd line.
try:
    os.chdir(location)
except:
...
# Check file provided is a csv file.
if ".csv" in file_name:
    try:
...
# Initial error checking of essential entries from parameter file. If any of 
# these are blank then the program will stop.
...
for x in par_list_G4_values:
    if pd.isnull(x) == True:
        print("\nEssential entry for Overall title on/off in Parameter file "
              " is missing. Enter 'on' or 'off'.") 
        sys.exit()
...
# Check parameter file colour entries. Maximum colour number for later 
# function. Colours are from 1-23. This can be added to by the user if 
# required (look up matplotlib colours). Just add more to the function 
# 'colour' below and change the maximum colour number here.
col_max = 23 

###############################################################################
###############################################################################
# Change working directory and load in raw data to be plotted from input file. 
...
# Load in extra input file if required. File with information for plots with 
# multiple entries.
extra_yn = str(par_dict["Extra input file name**"])
...
if extra_yn != "none":
    try:
        ...
        print(f"\nProblem loading in Input file. Is {extra_input_filename} "
              "the correct name and in correct location ? Check location and " 
              "spelling of name.")
        sys.exit()
        
# Remove columns in dataframes if have no values other than zero.
...
# Remove columns in dataframes if have no values other than zero (as above 
# for 'extra' file if required).
if extra_yn != "none":
        ...
        for x in values:
            if pd.isnull(x) == True:
                print(f"\nEntry missing in Extra input file in the {name} "
                      "column.There should be no blank entries. If it is a "
                      "numeric value use a zero.")
                sys.exit()
...
# Obtain list of titles of taxa to be plotted from the input files (standard 
# and extra).
data_list = [col for col in data.columns]

if data_list [1] != "Depth" or data_list [-1] != "Zones":
    print("\nThe second and last column of the input file must be the 'Depth' "
          "and 'Zones' columns. One or both are incorrect. Make sure there "
          "are no spaces before or after the word 'Depth' or 'Zones' See "
...
if extra_yn != "none":
    ...
    if data_list_extra [1] != "Depth" or data_list_extra [-1] != "Zones":
        print("\nThe second and last column of the extra input file must be "
              "the 'Depth' and 'Zones' columns. One or both are incorrect. "
              "Make sure there are no spaces before or after the word 'Depth'"
              " or 'Zones' See manual for correct setup.")
        sys.exit()
...
# Get plot items and associated graph types.
if zones_on_off == "off":
...
# 7 is a percentage plot (only 2 of these are possible at present).
...
# Error checking for plot type number 7. Only two instances allowed at 
# present.
if sum(value == 7 for value in plot_type.values()) > 2:
    print("\nOnly 2 percentage style plots are available at present. More "
...
###############################################################################
# Get line colour values for lines for graphs 2, 3, 4, and 5 from 
# input files (see manual).
...
###############################################################################
# Obtain colour value for taxon title colour from 1-23 (see manual).
...
###############################################################################
# Obtain bold value for taxon title value of 0 or 1.
...
###############################################################################
# Obtain vertical spine width value for taxon. If required suggest values
# between 0.5 and 2. 
...
###############################################################################
# Obtain vertical spine style value for taxon. If required values between 
# 1-4 see manual for codes.
...
###############################################################################
# Obtain vertical spine colour value for taxon. if required values between 
# 1-23. See manual for colour codes.
...
###############################################################################
# Obtain left spine width value for taxon. If rquired suggest values 
# between 0.5 and 2.
...
###############################################################################
# Obtain left spine style value for taxon.
...
###############################################################################
# Obtain left spine colour value for taxon. If required values between 
# 1 - 23. See manual for colour codes. 
...
###############################################################################
# Obtain right spine width value for taxon. If required suggest values 
# between 0.5 and 2.
...
###############################################################################
# Obtain right spine style value for taxon. If required values between 1 - 4. 
...
###############################################################################
# Obtain right spine colour value for taxon. If required values between 
# 1 - 23. See manual for colour codes. 
...
###############################################################################
# Obtain x major tick colour value for taxon. If required values between 
# 1 - 23. See manual for colour codes black is 1.
...
###############################################################################
# Obtain x minor tick colour value for taxon. if required values between 
# 1 - 23. See manual for colour codes black is 1.
...
###############################################################################
# Obtain y major tick colour value for taxon. If required values between 
# 1 - 23. See manual for colour codes black is 1.
...
###############################################################################
# Obtain y minor tick colour value for taxon. If required values between 
# 1 - 23. See manual for colour codes black is 1. 
...
###############################################################################
# Obtain whether taxa is to have exaggeration or not. 0 is no exaggeration, 
# any other number is number to multiply original to get exaggeration.
...
###############################################################################
# If exaggeration is required this dictates the type, either graph type 3 or 
# 4 is permitted.
...
###############################################################################
# Obtain colour code of exaggeration fill. If required values between 1 - 23. 
# See manual for colour codes black is 1.
...
###############################################################################
# Obtain transparency of exaggeration fill. If required values between 0 - 1. 
# 0 fully tranparent, 1 not at all transparent.
...
###############################################################################
# Obtain colour code of exaggeration line, if required values between 
# 1 - 23. See manual for colour codes black is 1.
...
###############################################################################
# Obtain line width for exaggeration line. If required suggest values 
# between 0.5 and 2.
...
###############################################################################
# Obtain line style for exaggeration line. If required suggest values 
# between 1 - 4. See manual for codes.
...
###############################################################################
# Obtain whether percentage plot 1 is required.
perc_plot_1 = data.iloc[36:37,1::].transpose()
perc_plot_1 = perc_plot_1.drop("Depth")

if zones_on_off == "off":
    perc_plot_1 = perc_plot_1.drop("Zones")
    
perc_plot_1 = perc_plot_1.reset_index()
perc_plot_1.rename(columns = {"index": "TAXA"}, inplace = True)
perc_plot_1.rename(columns = {1: "PERC_Y_N"}, inplace = True)
perc_plot_1 = perc_plot_1.sort_values("TAXA")

# Create dictionary of above.
taxa_plot_perc_plot_1, taxon_plot_perc_plot_1 = \
                       list(perc_plot_1.iloc[:,0]), \
                       list(perc_plot_1.iloc[:,1])
taxa_perc_plot_1 = {taxa_plot_1_percs_plot: taxon_plot_1_percs_plot for
                    taxa_plot_1_percs_plot, taxon_plot_1_percs_plot \
                    in zip(taxa_plot_perc_plot_1, taxon_plot_perc_plot_1)}

perc_plot_1_uni = np.unique(taxon_plot_perc_plot_1)
perc_plot_1_uni = list(perc_plot_1_uni)
if 0.0 in perc_plot_1_uni:
    perc_plot_1_uni.remove(0.0)

###############################################################################
# Obtain whether percentage plot 2 is required.
perc_plot_2 = data.iloc[37:38,1::].transpose()
perc_plot_2 = perc_plot_2.drop("Depth")

if zones_on_off == "off":
    perc_plot_2 = perc_plot_2.drop("Zones")
    
perc_plot_2 = perc_plot_2.reset_index()
perc_plot_2.rename(columns = {"index": "TAXA"}, inplace = True)
perc_plot_2.rename(columns = {1: "exag LS"}, inplace = True)
perc_plot_2 = perc_plot_2.sort_values("TAXA")

# Create dictionary of above.
taxa_plot_perc_plot_2, taxon_plot_perc_plot_2 \
                        = list(perc_plot_2.iloc[:,0]), \
                          list(perc_plot_2.iloc[:,1])
taxa_perc_plot_2 = {taxa_plot_2_percs_plot: taxon_plot_2_percs_plot for \
                    taxa_plot_2_percs_plot, taxon_plot_2_percs_plot in \
                    zip(taxa_plot_perc_plot_2, taxon_plot_perc_plot_2)}

perc_plot_2_uni = np.unique(taxon_plot_perc_plot_2)
perc_plot_2_uni = list(perc_plot_2_uni)

if 0.0 in perc_plot_2_uni:
    perc_plot_2_uni.remove(0.0)

###############################################################################
###############################################################################
# Create a parameter to 'scale' the y axis of each plot correctly, 
# 'the abundance' axis.

# Need to know if any non standard scaling graphs are required before can do 
# this.
...
# Create a dummy diff list with all elements to update later to preserve the 
# order.
...
# Account for whether exaggerations are requested.
if exag["exag"].max() > 0: 
    new_max_list = [] 
...
diff_list_ratios_dict_1 = {k: v for k, v in zip(taxa_list, diff_list_ratios)}
...
# Remove plot style and colour indexes from the input file ready for plotting.
...
###############################################################################
###############################################################################
# Read in groups for percentage data if percentage plots are required.
perc_plot_1_on_off = par_dict["Percentage stack plot 1 on/off**"]. \
                              strip().lower()
perc_plot_2_on_off = par_dict["Percentage stack plot 2 on/off**"]. \
                              strip().lower()

# Obtain parameters for percentage plot 1 if required and error check entries.
if pd.isnull(perc_plot_1_on_off) == True or perc_plot_1_on_off not in \
    ["on", "off"]:
    print("\nPercentage 1 stack plot on/off in Parameter file "
          "requires an entry of 'on' or 'off'.")
    sys.exit()
    
if perc_plot_1_on_off == "on":
    
    if max(taxa_perc_plot_1.values()) <= 0:
        print("\nPercentage stack plot 1 has been requested in the "
              "the Parameter file but there is no groupings in the input "
              "file.")
        sys.exit()
        
    try:
        perc_plot_1_title = par_dict \
        ["Percentage stack plot title 1"].strip()
        
        if pd.isnull(perc_plot_1_title) == True:
            print("\nPercentage stack plot title 1 entry in Parameter file "
                  "is empty. Is an entry required ?")
    except:
        print("\nPercentage stack plot title 1 entry in Parameter file is "
              "missing or incorrect.")
        sys.exit()
        
    # Check colour entries in parameter files when provided as list.
    try:
        perc_plot_1_colours = par_dict["Percentage stack plot col list "
                                       "1"].replace(" ","").split(",")
        perc_plot_1_colours = [float(x) for x in perc_plot_1_colours]
        
    except:
        print("\nEntry for Percentage stack plot col list 1 in Parameter "
              "file is missing or incorrect.")
        sys.exit()
        
    if any(pd.isnull(perc_plot_1_colours)) == False:
        if any (i < 0 or i > col_max for i in perc_plot_1_colours):
            print("\n A colour entry for Percentage stack plot col list in "
                  "Parameter file is out of bounds.")
            sys.exit()
        
    try:
        perc_plot_1_size = float(par_dict["Percentage stack plot size 1"])
        
        if pd.isnull(perc_plot_1_size) == True:
            print("\nPercentage stack plot 1 entry in Parameter file is "
                  "required.")
            sys.exit()
    except:
        print("\nPercentage stack plot size 1 entry in Parameter file is "
              "missing or incorrect.")
        sys.exit()
        
    try:
        perc_plot_1_lw = float(par_dict \
                               ["Percentage stack plot line width 1"])
            
        if pd.isnull(perc_plot_1_lw) == False:
            try:
                perc_plot_1_line_colour = float(par_dict\
                                                ["Percentage stack "
                                                "plot line colour 1"])

                if perc_plot_1_line_colour < 0 or \
                   perc_plot_1_line_colour > col_max or \
                   pd.isnull(perc_plot_1_line_colour) == True:
                    print("\nPercentage stack plot line colour 1 entry in "
        ...
        print("\nPercentage stack plot line width 1 entry in Parameter file "
              "is missing or incorrect if not required use zero as entry.")
        sys.exit()
        
    perc_plot_1_line_colour = float(par_dict["Percentage stack "
                                             "plot line colour 1"])
    
    if perc_plot_1_line_colour <= 0 or perc_plot_1_line_colour > \
            col_max:
        print("\nPercentage stack plot line colour 1 entry in Parameter file "
              "is missing or incorrect.")
        sys.exit()

    perc_plot_1_yn = par_dict["Percentage stack plot title 1"].strip()
    
    try:
        perc_calc_1 = par_dict["Percentage stack plot calculation 1"]. \
                              strip().lower()
                              
        if pd.isnull(perc_calc_1) == True or perc_calc_1 not in \
                ["yes","no"]:
            print("\nEntry for Percentage stack plot calculation 1 is "
                  "required. Entry should be yes or no.")
            sys.exit()
            
    except:
        print("\nEntry for Percentage stack plot calculation 1 entry in the "
              "Parameter file is missing or incorrect.") 
        sys.exit()
        
    if len(perc_plot_1_colours) != len(perc_plot_1_uni):
        print("\nNumber of Percentage stack 1 plot groups are different to "
              "number of colours prescribed in Parameter file.")
        sys.exit()

# Obtain parameters for percentage plot 2 if required and error check entries.
if perc_plot_2_on_off == "on":
    if max(taxa_perc_plot_2.values()) <= 0:
        print("\nPercentage stack plot 2 has been requested in the "
              "Parameter file but there is no groupings in the input file.")
        sys.exit()
        
    try:
        perc_plot_2_title = par_dict \
        ["Percentage stack plot title 2"].strip()
        
        if pd.isnull(perc_plot_2_title) == True:
            print("\nPercentage stack plot title 2 entry is empty. Is an "
                  "entry required ?")
            sys.exit()
            
    except:
        print("\nPercentage stack plot title 2 entry in Parameter file is "
              "missing or incorrect.")
        sys.exit()
        
    # Check colour entries in parameter files when provided as list.
    try:
        perc_plot_2_colours = par_dict["Percentage stack plot col "
                                       "list 2"].replace(" ","").split(",")
        perc_plot_2_colours = [float(x) for x in perc_plot_2_colours]
        
        if any(pd.isnull(perc_plot_2_colours)) == False:
            if any (i < 0 or i > col_max for i in perc_plot_2_colours):
                print("\nA colour entry for Zone line col list in Parameter "
                      "file is out of bounds.")
                sys.exit()
                
    except:
        print("\nEntry for Percentage stack plot col list 2 in Parameter "
              "file is missing or incorrect. Make sure all required entries "
              "are filled.")
        sys.exit()
        
    try:
        perc_plot_2_size = float(par_dict["Percentage stack plot size 2"])
        if pd.isnull(perc_plot_2_size) == True:
            print("\nPercentage stack plot 2 entry in Parameter file is "
                  "required.")
            sys.exit()
            
    except:
        print("\nPercentage stack plot size 2 entry in Parameter file is "
              "missing or incorrect.")
        sys.exit()
        
    try:
        perc_plot_2_lw = float(par_dict \
                               ["Percentage stack plot line width 2"])
        if pd.isnull(perc_plot_2_lw) == False:
            try:
                perc_plot_2_line_colour = float(par_dict\
                                                ["Percentage stack "
                                                "plot line colour 2"])
                    
                if perc_plot_2_line_colour < 0 or \
                    perc_plot_2_line_colour > col_max or \
                    pd.isnull(perc_plot_2_line_colour) == True:
                    print("\nPercentage stack plot line colour 2 in "
                          "Parameter file is missing or out of bounds.")
                    sys.exit()
                    
            except:
                print("\nPercentage stack plot line colour 2 entry in "
                      "Parameter file is missing or incorrect.")
                sys.exit()
    except:
        print("\nPercentage stack plot line width 2 entry in Parameter file "
              "is missing or incorrect if not required use zero as entry.")
        sys.exit()

    perc_plot_2_line_colour = float(par_dict["Percentage stack "
                                             "plot line colour 2"])

    if perc_plot_2_line_colour <= 0 or perc_plot_2_line_colour > \
            col_max:
        print("\nPercentage stack plot line colour 2 entry in Parameter file "
              "is missing or incorrect.")
        sys.exit()

    perc_plot_2_yn = par_dict["Percentage stack plot title 2"].strip()
    
    try:
        perc_calc_2 = par_dict["Percentage stack plot calculation 2"]. \
                              strip().lower()
                              
        if pd.isnull(perc_calc_2) == True or perc_calc_2 not in \
                ["yes","no"]:
            print("\nEntry for Percentage stack plot calculation 2 in "
                  "Parameter file is required. Entry should be 'yes' or "
                  "'no'.")
            sys.exit()
    except:
        print("\nEntry for Percentage stack plot calculation 2 entry in "
              "Parameter file is missing or incorrect.") 
        sys.exit()

    if len(perc_plot_2_colours) != len(perc_plot_2_uni):
        print("\nNumber of Percentage stack 2 plot groups are different to "
              "number of colours prescribed in Parameter file.")
        sys.exit()

# Determine number of perc plots asked for by user 0, 1 or 2.
perc_plots_yn = [perc_plot_1_on_off, perc_plot_2_on_off]
num_perc_plots = sum([1 if x !="off" and pd.isnull(x) == False \
                      else 0 for x in perc_plots_yn])
    
# Create marker ready for later in the program
num_perc = 1
        
if num_perc_plots > 0 and perc_plot_1_on_off =="off":
    print("\nUse available percentage stack plot 1 entries before "
          "percentage stack plot 2 entries if only one percentage stack plot "
          "required.")
    sys.exit()

# Prep data for later percentage plots if required.
if num_perc_plots > 0:
    ...
    data_perc_1 = list(data_3.iloc[::,0])
    data_perc_2 = list(data_3.iloc[::,1])
    
###############################################################################
    # Percentage plot 1 can have up to 5 groups as part of the percentage 
    ...
    if perc_plot_1_on_off == "on":
        ...
        # Create a ratio for the percentage plot so is correctly scaled 
        # on y axis.
        diff_list_ratios_dict[perc_plot_1_title] = 1 * (perc_plot_1_size)
        
        a = 2
        
        # Get data for stack plots and error check.
        for x in range (length - 2):
            perc_list_1_1, perc_list_1_2, perc_list_1_3, \
            perc_list_1_4, perc_list_1_5 = [],[],[],[],[]
            
            for values, grouping in zip(data_3.iloc[::,a], data_perc_1):
                if grouping == 1:
                    perc_list_1_1.append(values)
                if grouping == 2:
                    perc_list_1_2.append(values)
                if grouping == 3:
                    perc_list_1_3.append(values)
                if grouping == 4:
                    perc_list_1_4.append(values)
                if grouping == 5:
                    perc_list_1_5.append(values)
                    
                if grouping > 5 or pd.isnull(grouping) \
                    == True:
                        print("\nPercentage stack plot 1 groupings are "
                              "limited to 5 groups maximim at present. "
                              "Error in grouping numbers. If not required "
                              "add a zero.")
                        sys.exit()

            perc_1_sums_1.append(sum(perc_list_1_1))
            perc_1_sums_2.append(sum(perc_list_1_2))
            perc_1_sums_3.append(sum(perc_list_1_3))
            perc_1_sums_4.append(sum(perc_list_1_4))
            perc_1_sums_5.append(sum(perc_list_1_5))
        ...
        if perc_calc_1 == "yes":
            perc_1_list = [perc_1_sums_1, perc_1_sums_2, perc_1_sums_3, \
                           perc_1_sums_4, perc_1_sums_5]
            
            perc_1_all_sum = [v + w + x + y + z for v, w, x, y, z \
                              in zip(perc_1_sums_1, perc_1_sums_2, \
                              perc_1_sums_3, perc_1_sums_4, perc_1_sums_5)]
            ...
            perc_1_list_calc = [perc_1_sums_1_calc,perc_1_sums_2_calc, \
                                perc_1_sums_3_calc, perc_1_sums_4_calc, \
                                perc_1_sums_5_calc]
            
            for w, x in zip(perc_1_list, perc_1_list_calc):
                for y, z in zip(w, perc_1_all_sum):
                    x.append(y/z * 100)

###############################################################################
    # Percentage plot 2 can have up to 5 groups as part of the percentage 
    # plots.can expand this if required by adding extra code.
    if perc_plot_2_on_off == "on":
        ...
        # Create a ratio for the percentage plot so is correctly scaled 
        # on y axis.
        diff_list_ratios_dict[perc_plot_2_title] = 1 * (perc_plot_2_size)
    
        a = 2
        
        # Get data for stack plots and error check.
        for x in range (length - 2):
            perc_list_2_1, perc_list_2_2, perc_list_2_3, perc_list_2_4, \
            perc_list_2_5 = [], [], [], [], []
            
            for values, grouping in zip(data_3.iloc[::,a], data_perc_2):
                if grouping == 1:
                    perc_list_2_1.append(values)
                if grouping == 2:
                    perc_list_2_2.append(values)
                if grouping == 3:
                    perc_list_2_3.append(values)
                if grouping == 4:
                    perc_list_2_4.append(values)
                if grouping == 5:
                    perc_list_2_5.append(values) 
            ...
            perc_2_sums_1.append(sum(perc_list_2_1))
            perc_2_sums_2.append(sum(perc_list_2_2))
            perc_2_sums_3.append(sum(perc_list_2_3))
            perc_2_sums_4.append(sum(perc_list_2_4))
            perc_2_sums_5.append(sum(perc_list_2_5))
        ...
        if perc_calc_2 == "yes":
            perc_2_list = [perc_2_sums_1, perc_2_sums_2, perc_2_sums_3,\
                           perc_2_sums_4, perc_2_sums_5]
            
            perc_2_all_sum = [v + w + x + y + z for v, w, x, y, z \
                              in zip(perc_2_sums_1, perc_2_sums_2, \
                              perc_2_sums_3, perc_2_sums_4, perc_2_sums_5)]
            ...
            perc_2_list_calc = [perc_2_sums_1_calc, perc_2_sums_2_calc, \
                                perc_2_sums_3_calc, perc_2_sums_4_calc, \
                                perc_2_sums_5_calc]
            
            for w, x in zip(perc_2_list, perc_2_list_calc):
                for y, z in zip(w, perc_2_all_sum):
                    x.append(y/z * 100)

###############################################################################
###############################################################################
# Obtain axis parameters from input file such as axis limits, intervals 
...
try:
    ...
    print("\nProblem with font style entry in Parameter file Group 3. Check "
          "entry for format and spelling.")
    sys.exit()
    
Font_check = ["arial", "calibri", "dejavu sans", "times new roman"]

if font_style not in Font_check:
    print("\nCheck font style entry in Parameter file. Is not one of the 4 "
          "presently available.")
    sys.exit()
...
try:
    ...
    print("\nProblem with Parameter file Group 2 entry. Check entry. Is it "
          "numeric?")
    sys.exit()
...
try:
    ...
    print("\nProblem with X limit top and X limit base entries in Parameter "
          "file. Check these entries are numeric and not strings.")
    sys.exit()
...
try:
    ...
    print("\nCheck Space between plots entry in Parameter file. Check entry "
          "and format.")
    sys.exit()
...
try:
    ...
    y_major_int_0 = int(par_dict["Y major tick interval**"].replace(" ",""))
except:
...
try:
    x_major_tick_len = float(par_dict["X major tick length**"].replace(" ",""))
    y_major_tick_len = float(par_dict["Y major tick length**"].replace(" ",""))
    x_major_tick_wid = float(par_dict["X major tick width**"].replace(" ",""))
    y_major_tick_wid = float(par_dict["Y major tick width**"].replace(" ",""))
except:
    print("\nProblem with X major tick length, Y major tick length, X major "
          "tick width or Y major tick width in Parameter file. Check these "
          "entries are numeric.")
    sys.exit()    
...
try:
    ...
    print("\nProblem with X ticks depth axis only, Y ticks both ends of "
          "plot on/off, X minor ticks on/off or Y minor ticks on/off entries "
          "in Parameter file. Check these entries are strings and are either "
          "'on' or 'off'.")
    sys.exit() 
...
if x_minor_ticks_on_off == "on":
    try:
        ...
        print("\nProblem with X minor interval, X minor tick length or X "
              "minor tick width in Parameter file. Check these entries are "
              "numeric.")
        sys.exit()         
...
if y_minor_ticks_on_off == "on":
    try:
        y_minor_int_0 = int(par_dict["Y minor tick interval"].replace(" ",""))
        y_minor_tick_wid = float(par_dict["Y minor tick width"]. \
                                 replace(" ",""))        
        y_minor_tick_len = float(par_dict["Y minor tick length"]. \
                                 replace(" ","")) 
    except:
        print("\nProblem with Y minor tick interval, Y minor tick width or Y "
              "minor tick length in Parameter file. Check these entries are "
              "numeric.")
        sys.exit()     
...
# Initial checks of Overall title entries.
...
if title_text_on_off == "on":
    ...
    if title_text_colour < 1 or title_text_colour > col_max:
        print("\nOverall title text colour entry in Parameter file is out of "
              "bounds or incorrect. See manual for range of colour codes. "
              "These are presently 1-23.")
        sys.exit()
    ...
    if title_text_bold not in ["on", "off"]:
        print("\nOverall title text bold on/off in Parameter file needs to "
              "be either 'on' or 'off'.")
        sys.exit()
    ...
    if title_rotation < 0 or title_rotation > 360:
        print("\nOverall title rotation in Parameter file is out of bounds "
              "or incorrect.")
        sys.exit()
...
# Initial checks of footer entries.
...
if footer_text_on_off == "on":
    ...
    if footer_text_bold not in ["on", "off"]:
        print("\nOverall footer text bold on/off in Parameter file needs to "
              "be either 'on' or 'off'.")
        sys.exit() 
                               
    try:                                                                
        footer_font_size = float(par_dict["Footer font size"].replace(" ",""))
        footer_rotation = float(par_dict["Footer rotation"].replace(" ",""))
    except:
    ...
    try:
        footer_y_pos = float(par_dict["Footer Y position"].replace(" ",""))
        footer_x_pos = float(par_dict["Footer X position"].replace(" ",""))
    except:
...
try:    
    y_title_rotation = float(par_dict["Y title rotation**"].replace(" ",""))
    x_title_rotation = float(par_dict["X title rotation**"].replace(" ",""))
    x_title_fontsize  = float(par_dict["X title font size**"].replace(" ",""))
    y_title_fontsize = float(par_dict["Y title font size**"].replace(" ",""))
...
if x_title_text_bold not in ["on", "off"]:
    print("\nX title bold on/off entry in Parameter file needs to be either "
          "'on' or 'off'.")
    sys.exit()
...
if any(i >360 or i < 0 for i in x_y_lab_rot) == True:
    print("\nX label rotation or Y label rotation are out of bounds of 0 to "
          "360 degrees range. Check numbers and formats.")
    sys.exit()
    
###############################################################################
###############################################################################
# Odd scaling taxa. Find what the user wants to use as non_std_scaling and get  
# associated parameters and check. Can be up to 5 non_std_scaling graphs at 
# present. Could be expanded in the future but doubt would be required.
non_std_list = [non_std_scaling_1, non_std_scaling_2, non_std_scaling_3,\
                non_std_scaling_4, non_std_scaling_5]
...
if non_std_scaling_5 != "none" :
    if pd.isnull(non_std_scaling_5) == False:
...
if non_std_scaling_4 != "none" and non_std_scaling_5 == "none":
    if pd.isnull(non_std_scaling_5) == False and \
        pd.isnull(non_std_scaling_4) == False:
        diff_list_ratios_dict[non_std_scaling_4] = 1
        diff_list_ratios_dict[non_std_scaling_3] = 1
        diff_list_ratios_dict[non_std_scaling_2] = 1
        diff_list_ratios_dict[non_std_scaling_1] = 1
...
if non_std_scaling_3 != "none" and non_std_scaling_4 == "none":
    if pd.isnull(non_std_scaling_3) == False and \
        pd.isnull(non_std_scaling_4) == False:
        diff_list_ratios_dict[non_std_scaling_3] = 1
        diff_list_ratios_dict[non_std_scaling_2] = 1
        diff_list_ratios_dict[non_std_scaling_1] = 1
    
if non_std_scaling_2 != "none" and non_std_scaling_3 == "none":
    if pd.isnull(non_std_scaling_2) == False and \
        pd.isnull(non_std_scaling_3) == False:
        diff_list_ratios_dict[non_std_scaling_2] = 1
        diff_list_ratios_dict[non_std_scaling_1] = 1
        
if non_std_scaling_1 != "none" and non_std_scaling_2 == "none":
    if pd.isna(non_std_scaling_1) == False and \
        pd.isna(non_std_scaling_2) == False:
        diff_list_ratios_dict[non_std_scaling_1] = 1
...
if non_std_scaling_1 != "none":
    try:
        ...
        non_std_spine_on_off_1 = str(par_dict["NSC 1 spine on/off"]). \
                                              replace(" ","").lower()
        non_std_scaling_1_size = float(par_dict["NSC 1 size"].replace(" ",""))
        ...
        print("\nCheck NSC 1 y major tick interval, NSC 1 y minor tick "
              "interval, NSC 1 y min, NSC 1 y max, NSC 1 spine on/off, NSC 1 "
              "size or NSC spine start 1. A problem exists with one of these "
              "entries. Check entry and format.")
        sys.exit()
        
    if non_std_spine_on_off_1 not in ["on","off"]:
        print("\nEntry for NSC 1 spine on/off must be either 'on' or 'off'. " 
              "Check entry.")
        sys.exit()
    ...
    for k,v in non_std_scaling_dict_1.items():
        if pd.isnull(v) == True:
            print(f"\nEntry missing for {k} in Group 14 in Parameter file.")
            sys.exit() 
        if k == "NSC 1 y minor tick interval" and v == str(0) and \
                 y_minor_ticks_on_off == "on":
            print("\nY minor ticks have been selected as on so zero is not "
                  "permitted as entry for NSC 1 y min. Choose sensible "
                  "number that is a division of the interval stated in the "
                  "NSC 1 y max entry.")
            sys.exit()
...
if non_std_scaling_2 != "none":
    try:
        ...
        non_std_spine_on_off_2 = str(par_dict["NSC 2 spine on/off"]). \
                                              replace(" ","").lower()
        non_std_scaling_2_size = float(par_dict["NSC 2 size"].replace(" ",""))
        ...
        print("\nCheck NSC 2 y major tick interval, NSC 2 y minor tick "
              "interval, NSC 2 y min, NSC 2 y max NSC 2 spine on/off, NSC 2 "
              "size or NSC spine start 2. A problem exists with one of these "
              "entries. Check entry and format.")
        sys.exit()
        
    if non_std_spine_on_off_2 not in ["on","off"]:
        print("\nEntry for NSC 2 spine on/off must be either 'on' or 'off'. " 
              "Check entry.")
        sys.exit()
    ...
    for k,v in non_std_scaling_dict_2.items():
        ...
        if k == "NSC 2 y minor tick interval" and v == str(0) and \
                 y_minor_ticks_on_off == "on":
            print("\nY minor ticks have been selected as on so zero is not "
                  "permitted as entry for NSC 2 y min. Choose sensible "
                  "number that is a division of the interval stated in the "
                  "NSC 2 y max entry.")
            sys.exit()
...
if non_std_scaling_3 != "none":
    try:
        ...
        non_std_spine_on_off_3 = str(par_dict["NSC 3 spine on/off"]). \
                                              replace(" ","").lower()
        non_std_scaling_3_size = float(par_dict["NSC 3 size"].replace(" ",""))
        ...
        print("\nCheck NSC 3 y major tick interval, NSC 3 y minor tick "
              "interval, NSC 3 y min, NSC 3 y max NSC 3 spine on/off, NSC 3 "
              "size or NSC spine start 3. A problem exists with one of these "
              "entries. Check entry and format.")
        sys.exit()
        
    if non_std_spine_on_off_3 not in ["on","off"]:
        print("\nEntry for NSC 3 spine on/off must be either 'on' or 'off'. " 
              "Check entry.")
        sys.exit()
    ...
    for k,v in non_std_scaling_dict_3.items():
        ...
        if k == "NSC 3 y minor tick interval" and v == str(0) and \
                 y_minor_ticks_on_off == "on":
            print("\nY minor ticks have been selected as on so zero is not "
                  "permitted as entry for NSC 3 y min. Choose sensible "
                  "number that is a division of the interval stated in the "
                  "NSC 3 y max entry.")
            sys.exit()
...
if non_std_scaling_4 != "none":
    try:
        ...
        non_std_spine_on_off_4 = str(par_dict["NSC 4 spine on/off"]). \
                                              replace(" ","").lower()
        non_std_scaling_4_size = float(par_dict["NSC 4 size"].replace(" ",""))
        ...
        print("\nCheck NSC 4 y major tick interval, NSC 4 y minor tick "
              "interval, NSC 4 y min, NSC 4 y max NSC 4 spine on/off, NSC 4 "
              "size or NSC spine start 4. A problem exists with one of these "
              "entries. Check entry and format.")
        sys.exit()
        
    if non_std_spine_on_off_4 not in ["on","off"]:
        print("\nEntry for NSC 4 spine on/off must be either 'on' or 'off'. " 
              "Check entry.")
        sys.exit()
    ...
    for k,v in non_std_scaling_dict_4.items():
        ...
        if k == "NSC 4 y minor tick interval" and v == str(0) and \
                 y_minor_ticks_on_off == "on":
            print("\nY minor ticks have been selected as on so zero is not "
                  "permitted as entry for NSC 4 y min. Choose sensible "
                  "number that is a division of the interval stated in the "
                  "NSC 4 y max entry.")
            sys.exit()
...
if non_std_scaling_5 != "none":
    try:
        ...
        non_std_spine_on_off_5 = str(par_dict["NSC 5 spine on/off"]). \
                                              replace(" ","").lower()
        non_std_scaling_5_size = float(par_dict["NSC 5 size"].replace(" ",""))
        ...
        print("\nCheck NSC 5 y major tick interval, NSC 5 y minor tick "
              "interval, NSC 5 y min, NSC 5 y max NSC 5 spine on/off, NSC 5 "
              "size or NSC spine start 5. A problem exists with one of these "
              "entries. Check entry and format.")
        sys.exit()
        
    if non_std_spine_on_off_5 not in ["on","off"]:
        print("\nEntry for NSC 5 spine on/off must be either 'on' or 'off'. " 
              "Check entry.")
        sys.exit()
    ...
    for k,v in non_std_scaling_dict_5.items():
        ...
        if k == "NSC 5 y minor tick interval" and v == str(0) and \
                 y_minor_ticks_on_off == "on":
            print("\nY minor ticks have been selected as on so zero is not "
                  "permitted as entry for NSC 5 y min. Choose sensible "
                  "number that is a division of the interval stated in the "
                  "NSC 5 y max entry.")
            sys.exit()
...
###############################################################################
###############################################################################
# RC ages if supplied. Bring in all required parameters and check what user 
# has entered.
...
if rc_ages_on_off == "on":
    ...
    if pd.isnull(rc_age_title_on_off) == True:
        print("\nEntry for RC age title on/off is required. Entry should be "
              "'on' or off'.")
        sys.exit()
    ...
    try:
        ...
        print("\nEntry for RC age label position offset in Parameter file is "
              "required.")              
        sys.exit()        
    ...
    try:
        ...
        print("\nProblem with one of the entries in Parameter file for "
              "either RC age label font size, RC age labels colour, RC age "
              "labels bold on/off or RC age labels rotation entries. Check "
              "formats are correct.")
        sys.exit()
    ...
    if rc_age_labels_rotation < 0 or rc_age_labels_rotation > 360:
        print("\nEntry for RC age labels rotation in Parameter file is not "
              "between 0 and 360.")
        sys.exit()                 
    ...
    if len(rc_age_labels) != len(rc_age_depths):
        print("")
        print("\nError in RC label depths in Parameter file. Unequal number "
              "of depths and labels.")
        sys.exit()
...
# Load in data for RC age title if it is specified as being 'on' and check.
    if rc_age_title_on_off == "on":
        ...
        try:
            ...
            print("\nProblem with one of the entries in Parameter file for "
                  "either RC age title depth position, RC age title offset "
                  "position, RC age title font size, RC age title colour, "
                  "RC age title bold on/off or RC age title rotation entries."
        ...
        if rc_age_title_colour < 0 or rc_age_title_colour > col_max:
            print("\nEntry for RC age title colour in Parameter file is out "
                  "of bounds.")
            sys.exit()
            
        if rc_age_title_bold not in ["on", "off"]:
            print("\nEntry for RC age title bold on/off in Parameter file is "
                  "required. Enter 'on' or 'off'.")
            sys.exit()            
...
###############################################################################
###############################################################################
# Date line ages if supplied bring in all required parameters and check.
...
if int_ages_on_off not in ["on", "off"]:
    print("\nIncorrect entry for INT ages on or off in Parameter file. If "
          "not using feature provide an entry of 'off' in Parameter file. "
          "Entries should be 'on' or 'off'.")
    sys.exit()

# Obtain INT parameters and check entries.
if int_ages_on_off == "on":
    ...
    if int_age_title_on_off not in ["on","off"]:
        print("\nINT age title on/off entry should be either 'on' or 'off'.")
        sys.exit()
    ...
    try:
        ...
        print("\nCheck entry and format for INT age labels position offset "
              "in Parameter file.")
        sys.exit()        
        
    if pd.isnull(int_age_location_offset) == True:
        print("\nEntry for INT age labels position offset in Parameter file "
              "is required.")
        sys.exit()
    
    try:
        int_age_labels = par_dict["INT age labels"].replace(" ","").split(",")
    except:
    ...
    try:
        ...
        print("\nProblem with entry for INT age labels font size, INT age "
              "labels colour, INT age labels bold on/off, or INT age labels "
              "rotation. Check entries and formats.")
        sys.exit()
    ...
    if int_age_labels_bold not in ["on", "off"]:
        print("\nEntry for INT age labels bold on/off in Parameter file is "
              "required. Enter 'on' or 'off'.")
        sys.exit()            
        
    if int_age_labels_rotation < 0 or int_age_labels_rotation > 360:
        print("\nEntry for INT age labels rotation in Parameter file is not "
              "between 0 and 360.")
        sys.exit()
    ...
    if any(pd.isnull(int_age_depth_labels)) == True:
        print("\nThere is a missing value or an empty space in the INT age "
              "depth labels positions entry in Parameter file.")
        sys.exit()    
    ...
    if int_age_title_on_off == "on":
        ...
        try:
            ...
            print("\nProblem with INT age title depth position, INT age "
                  "title offset position, INT age title font size or INT age "
                  "title colour entry. Check these entries and their format.")
            sys.exit()
            
        if int_age_title_colour < 0 or int_age_title_colour > col_max:
            print("\nEntry for INT age title colour in Parameter file is out "
                  "of range.")
            sys.exit()
        ...
        if int_age_title_bold not in ["on", "off"]:
            print("\nEntry for INT age title bold on/off in Parameter file "
                  "required to be 'on' or 'off'.")
            sys.exit()
    ...
    try:
        int_lines_colour = float(par_dict["INT lines colour"].replace(" ",""))
        int_lines_offset = float(par_dict["INT lines offset"].replace(" ",""))
        int_lines_width = float(par_dict["INT lines width"].replace(" ",""))
...
###############################################################################
###############################################################################
# Grouping annotation data 1-10. Bring in required parameters for any 
# grouping annotations required for possible groups 1-10 and check.
try:
...
if group_anno_1 == "on":
    ...
    try:
        group_anno_1_title_colour = float(par_dict["Grouping annotation 1 "
                                                   "title colour"])
        group_anno_1_title_font_size = float(par_dict["Grouping annotation 1 "
                                                      "title font size"])    
        ...
        group_anno_1_line_y_offset_start = float(par_dict["Grouping "
                                                          "annotation 1 "
                                                          "line start y"])
        group_anno_1_line_x_offset_end = float(par_dict["Grouping annotation"
                                                        " 1 line end x"])
        group_anno_1_line_y_offset_end = float(par_dict["Grouping annotation "
                                                        "1 line end y"])
        group_anno_1_line_x_tag_end = float(par_dict["Grouping annotation 1 "
                                               "tag end x"])
        group_anno_1_line_y_tag_end = float(par_dict["Grouping annotation 1 "
                                                     "tag end y"])
        group_anno_1_corr = float(par_dict["Grouping annotation 1 tag "
                                           "correction"])
        group_anno_1_line_y_correction = float(par_dict["Grouping annotation"
                                                        " 1 line correction"])
    except:
        print("\nCheck entries for Group 1 annotation in Parameter file. One "
              "of the numeric entries is missing or erroneous. Check "
              "entries and formats.")
        sys.exit()
    ...
    if group_anno_1_title_colour < 0 or group_anno_1_title_colour > \
        ...
        print("\nEntry for Grouping annotation 1 title colour in "
              "Parameter file is out of bounds. Refer to manual for colour "
              "codes. Colour codes presently range from 1-23.")
        sys.exit()
...
if group_anno_2 == "on":
    ...
    try:
        ...
        print("\nCheck entries for Grouping annotation 2 start, "
              "Grouping annotation 2 title and Grouping annotation 2 title "
              "bold on/off. One of these is erroneous. Check entry and "
              "format.")
        sys.exit()
    ...
    try:
        group_anno_2_title_colour = float(par_dict["Grouping annotation 2 "
                                                   "title colour"])
        group_anno_2_title_font_size = float(par_dict["Grouping annotation 2 "
                                                      "title font size"])    
        ...
        group_anno_2_line_y_offset_start = float(par_dict["Grouping "
                                                          "annotation 2 "
                                                          "line start y"])
        group_anno_2_line_x_offset_end = float(par_dict["Grouping annotation"
                                                        " 2 line end x"])
        group_anno_2_line_y_offset_end = float(par_dict["Grouping annotation "
                                                        "2 line end y"])
        group_anno_2_line_x_tag_end = float(par_dict["Grouping annotation 2 "
                                               "tag end x"])
        group_anno_2_line_y_tag_end = float(par_dict["Grouping annotation 2 "
                                                     "tag end y"])
        group_anno_2_corr = float(par_dict["Grouping annotation 2 tag "
                                           "correction"])
        group_anno_2_line_y_correction = float(par_dict["Grouping annotation"
                                                        " 2 line correction"])
    except:
...
if group_anno_3 == "on":
    ...
    try:
        group_anno_3_title_colour = float(par_dict["Grouping annotation 3 "
                                                   "title colour"])
        group_anno_3_title_font_size = float(par_dict["Grouping annotation 3 "
                                                      "title font size"])    
        ...
        group_anno_3_line_y_offset_start = float(par_dict["Grouping "
                                                          "annotation 3 "
                                                          "line start y"])
        group_anno_3_line_x_offset_end = float(par_dict["Grouping annotation"
                                                        " 3 line end x"])
        group_anno_3_line_y_offset_end = float(par_dict["Grouping annotation "
                                                        "3 line end y"])
        group_anno_3_line_x_tag_end = float(par_dict["Grouping annotation 3 "
                                               "tag end x"])
        group_anno_3_line_y_tag_end = float(par_dict["Grouping annotation 3 "
                                                     "tag end y"])
        group_anno_3_corr = float(par_dict["Grouping annotation 3 tag "
                                           "correction"])
        group_anno_3_line_y_correction = float(par_dict["Grouping annotation"
                                                        " 3 line correction"])
    except:
        print("\nCheck entries for Group 3 annotation in Parameter file. One "
              "of the numeric entries is missing or erroneous. Check "
              "entries and formats.")
        sys.exit()
...
if group_anno_4 == "on":
    ...
    try:
        group_anno_4_title_colour = float(par_dict["Grouping annotation 4 "
                                                   "title colour"])
        group_anno_4_title_font_size = float(par_dict["Grouping annotation 4 "
                                                      "title font size"])    
        ...
        group_anno_4_line_y_offset_start = float(par_dict["Grouping "
                                                          "annotation 4 "
                                                          "line start y"])
        group_anno_4_line_x_offset_end = float(par_dict["Grouping annotation"
                                                        " 4 line end x"])
        group_anno_4_line_y_offset_end = float(par_dict["Grouping annotation "
                                                        "4 line end y"])
        group_anno_4_line_x_tag_end = float(par_dict["Grouping annotation 4 "
                                               "tag end x"])
        group_anno_4_line_y_tag_end = float(par_dict["Grouping annotation 4 "
                                                     "tag end y"])
        group_anno_4_corr = float(par_dict["Grouping annotation 4 tag "
                                           "correction"])
        group_anno_4_line_y_correction = float(par_dict["Grouping annotation"
                                                        " 4 line correction"])
    except:
...
if group_anno_5 == "on":
    ...
    try:
        group_anno_5_title_colour = float(par_dict["Grouping annotation 5 "
                                                   "title colour"])
        group_anno_5_title_font_size = float(par_dict["Grouping annotation 5 "
                                                      "title font size"])    
        group_anno_5_line_colour = float(par_dict["Grouping annotation 5 "
                                                  "line colour"])
        ...
        group_anno_5_line_y_offset_start = float(par_dict["Grouping "
                                                          "annotation 5 "
                                                          "line start y"])
        group_anno_5_line_x_offset_end = float(par_dict["Grouping annotation"
                                                        " 5 line end x"])
        group_anno_5_line_y_offset_end = float(par_dict["Grouping annotation "
                                                        "5 line end y"])
        group_anno_5_line_x_tag_end = float(par_dict["Grouping annotation 5 "
                                               "tag end x"])
        group_anno_5_line_y_tag_end = float(par_dict["Grouping annotation 5 "
                                                     "tag end y"])
        group_anno_5_corr = float(par_dict["Grouping annotation 5 tag "
                                           "correction"])
        group_anno_5_line_y_correction = float(par_dict["Grouping annotation"
                                                        " 5 line correction"])
    except:
...
if group_anno_6 == "on":
    ...
    try:
        group_anno_6_title_colour = float(par_dict["Grouping annotation 6 "
                                                   "title colour"])
        group_anno_6_title_font_size = float(par_dict["Grouping annotation 6 "
                                                      "title font size"])    
        ...
        group_anno_6_line_y_offset_start = float(par_dict["Grouping "
                                                          "annotation 6 "
                                                          "line start y"])
        group_anno_6_line_x_offset_end = float(par_dict["Grouping annotation"
                                                        " 6 line end x"])
        group_anno_6_line_y_offset_end = float(par_dict["Grouping annotation "
                                                        "6 line end y"])
        group_anno_6_line_x_tag_end = float(par_dict["Grouping annotation 6 "
                                               "tag end x"])
        group_anno_6_line_y_tag_end = float(par_dict["Grouping annotation 6 "
                                                     "tag end y"])
        group_anno_6_corr = float(par_dict["Grouping annotation 6 tag "
                                           "correction"])
        group_anno_6_line_y_correction = float(par_dict["Grouping annotation"
                                                        " 6 line correction"])
    except:
...
if group_anno_7 == "on":
    ...
    try:
        group_anno_7_title_colour = float(par_dict["Grouping annotation 7 "
                                                   "title colour"])
        group_anno_7_title_font_size = float(par_dict["Grouping annotation 7 "
                                                      "title font size"])    
        group_anno_7_line_colour = float(par_dict["Grouping annotation 7 "
                                                  "line colour"])
        ...
        group_anno_7_line_y_offset_start = float(par_dict["Grouping "
                                                          "annotation 7 "
                                                          "line start y"])
        group_anno_7_line_x_offset_end = float(par_dict["Grouping annotation"
                                                        " 7 line end x"])
        group_anno_7_line_y_offset_end = float(par_dict["Grouping annotation "
                                                        "7 line end y"])
        group_anno_7_line_x_tag_end = float(par_dict["Grouping annotation 7 "
                                               "tag end x"])
        group_anno_7_line_y_tag_end = float(par_dict["Grouping annotation 7 "
                                                     "tag end y"])
        group_anno_7_corr = float(par_dict["Grouping annotation 7 tag "
                                           "correction"])
        group_anno_7_line_y_correction = float(par_dict["Grouping annotation"
                                                        " 7 line correction"])
    except:
...
if group_anno_8 == "on":
    ...
    try:
        group_anno_8_title_colour = float(par_dict["Grouping annotation 8 "
                                                   "title colour"])
        group_anno_8_title_font_size = float(par_dict["Grouping annotation 8 "
                                                      "title font size"])    
        ...
        group_anno_8_line_y_offset_start = float(par_dict["Grouping "
                                                          "annotation 8 "
                                                          "line start y"])
        group_anno_8_line_x_offset_end = float(par_dict["Grouping annotation"
                                                        " 8 line end x"])
        group_anno_8_line_y_offset_end = float(par_dict["Grouping annotation "
                                                        "8 line end y"])
        group_anno_8_line_x_tag_end = float(par_dict["Grouping annotation 8 "
                                               "tag end x"])
        group_anno_8_line_y_tag_end = float(par_dict["Grouping annotation 8 "
                                                     "tag end y"])
        group_anno_8_corr = float(par_dict["Grouping annotation 8 tag "
                                           "correction"])
        group_anno_8_line_y_correction = float(par_dict["Grouping annotation"
                                                        " 8 line correction"])
    except:
...
if group_anno_9 == "on":
    ...
    try:
        group_anno_9_title_colour = float(par_dict["Grouping annotation 9 "
                                                   "title colour"])
        group_anno_9_title_font_size = float(par_dict["Grouping annotation 9 "
                                                      "title font size"])    
        group_anno_9_line_colour = float(par_dict["Grouping annotation 9 "
                                                  "line colour"])
        group_anno_9_line_width = float(par_dict["Grouping annotation 9 "
                                                 "line width"])    
        ...
        group_anno_9_line_y_offset_start = float(par_dict["Grouping "
                                                          "annotation 9 "
                                                          "line start y"])
        group_anno_9_line_x_offset_end = float(par_dict["Grouping annotation"
                                                        " 9 line end x"])
        group_anno_9_line_y_offset_end = float(par_dict["Grouping annotation "
                                                        "9 line end y"])
        group_anno_9_line_x_tag_end = float(par_dict["Grouping annotation 9 "
                                               "tag end x"])
        group_anno_9_line_y_tag_end = float(par_dict["Grouping annotation 9 "
                                                     "tag end y"])
        group_anno_9_corr = float(par_dict["Grouping annotation 9 tag "
                                           "correction"])
        group_anno_9_line_y_correction = float(par_dict["Grouping annotation"
                                                        " 9 line correction"])
    except:
...
if group_anno_10 == "on":
    ...
    try:
        ...
        group_anno_10_title = str(par_dict["Grouping annotation *10 "
                                           "title"]). strip()
        group_anno_10_title_bold = str(par_dict["Grouping annotation *10 "
                                     "title bold on/off"]).replace(" ",""). \
                                      lower()
    except:
    ...
    try:
        group_anno_10_title_colour = float(par_dict["Grouping annotation *10 "
                                                   "title colour"])
        group_anno_10_title_font_size = float(par_dict["Grouping annotation "
                                                       "*10 title font size"])    
        group_anno_10_line_colour = float(par_dict["Grouping annotation *10 "
                                                  "line colour"])
        group_anno_10_line_width = float(par_dict["Grouping annotation *10 "
                                                 "line width"])    
        ...
        group_anno_10_line_y_offset_start = float(par_dict["Grouping "
                                                          "annotation *10 "
                                                          "line start y"])
        group_anno_10_line_x_offset_end = float(par_dict["Grouping annotation"
                                                        " *10 line end x"])
        group_anno_10_line_y_offset_end = float(par_dict["Grouping annotation"
                                                        " *10 line end y"])
        group_anno_10_line_x_tag_end = float(par_dict["Grouping annotation "
                                                      "*10 tag end x"])
        group_anno_10_line_y_tag_end = float(par_dict["Grouping annotation "
                                                      "*10 tag end y"])
        group_anno_10_corr = float(par_dict["Grouping annotation *10 tag "
                                           "correction"])
        group_anno_10_line_y_correction = float(par_dict["Grouping annotation"
                                                        " *10 line "
                                                        "correction"])
    except:
...
if zones_on_off == "on":    
    # Obtain depths of zones from parameter file
    try:
    ...
    try:
        ...
        print("\nEntry required for Zone title font size in Parameter file. "
    ...
    # Valid entries are 1,2,3 or 4". See manula for definitions.
    try:
        zone_line_style = par_dict["Zone line style"].replace(" ",""). \
                                   split(",")
        zone_line_style  = [float(x) for x in zone_line_style]
    except:
        print("\nEntry required for Zone line style in Parameter file. Check "
              "entries.") 
        sys.exit()

    if any (i < 1 or i > 4 or pd.isnull(i) for i in \
            zone_line_style):
        print("\nZone line style list entries in Parameter file are out of "
              "bounds or missing.")
        sys.exit()        
    ...
    try:
        ...
        print("\nCheck all four of the Zone boundary line on/off entries in "
              "the Parameter file.")
        sys.exit()
    ...
    if any(pd.isnull(i) or i not in ["on","off"] for i in \
        ...
        print("\nCheck all four of the Zone boundary line on/off entries in "
              "the Parameter file. Entry should be 'on' or 'off'.")
        sys.exit()
    ...
    try:
        ...
        print("\nProblem with Zone boundary entry. Check all zone boundary "
              "entries and formats.")
        sys.exit()
    ...
    if zone_boundary_line_colour <= 0 or zone_boundary_line_colour > \
        ...
        print("\nZone boundary line colour entry in Parameter file is out of "
              "bounds or missing.")
        sys.exit()
           
    # Obtain parameters for individual zone labels and check.
    try:
    ...
    if zone_labels_on_off == "on":
        ...
        if zone_lab_rot < 0 or zone_lab_rot > 360:
            print("\nZone label rotation in Parameter file must be between 0 "
                  "and 360 degrees.")
            sys.exit()
        ...
        try:
            ...
            print("\nEntry for zone label position in Parameter file "
                  "required. If no correction required enter a zero. Check "
                  "entry and format.")
            sys.exit()

        # Obtain names of zones from parameter file.
        zones = []
        
        try:
            zone_labels = par_dict["Zone labels"].split(",")
        except:
            print("\nZone labels entry in Parameter file is incorrect. Check "
                  "entries. Is the entry blank?")
            sys.exit()
    ...
    if x_all_ticks == "off":
        ...
        if zone_x_tick_maj_colour < 0 or zone_x_tick_maj_colour > \
            ...
            print("\nZone X major tick colour entry in Parameter file is out "
                  "of bounds or missing.")
            sys.exit()  
        ...
        if x_minor_ticks_on_off == "on":
            try:
                ...
                print("\nZone X minor tick colour entry in Parameter file is "
                      "incorrect.")
                sys.exit() 
...
###############################################################################
# Obtain zone depths and upmost and deepest depths.
if zones_on_off == "on":
    zones = zone_lines
    zones.insert(0,0)
    ...
    # Create a central location for the zone labels in the zone column of the 
    # plot.
    zone_place = []
    ...
    if zones_on_off == "on":
        ...
        if len(zone_line_style) != len(zone_line_width) or \
            ...
            print("")
            print("\nError in zonation, too many or too few zone styles or "
                  "line widths in Parameter file.")
            sys.exit()
            
        if len(zone_line_width) != len(zone_line_colour) or \
            len(zone_line_width) != len(zone_lines[1:-1]):
            print("")
            print("\nError in zonation, too many or too few line colours or "
                  "line widths in Parameter file.")
            sys.exit()
        ...
        if len(zone_line_style) != len(zone_line_colour) or \
            ...
            print("")
            print("\nError in zonation, too many or too few zone styles or "
                  "colours in Parameter file.")
            sys.exit()

# Alter diff ratios if percentage stack plots are specified as well as zones
# and if without zones. Give default value of 1 for now. This is used to give 
...
# many taxa there are. The program is limited to 60 taxa. Beyond 60 it is not 
# really legible on an A4 piece of paper. 
...
# Function to turn colour number to colour text this can be added to 
# if required. See matplotlib available colours. If add extra colours increase
...
###############################################################################
###############################################################################
# Create individual plots for zone column if required and then each taxon in 
# turn. Order is dictated by order in the input csv file. Percentage stack 
# plots (only 2 available at present) need to be listed as the last columns 
...
###############################################################################
    ...
    if plot_type_1 == 0:
        ...
        if pd.isnull(zone_title) == False:
            ...
            graph.yaxis.labelpad = y_lab_gap * 18
        else:
...
###############################################################################
    ...
    if taxa_taxon_c_col[taxon] > col_max or \
        ...
        print("")
        print(f"\nError in plot title colour number designation for {taxon} "
              "in Input file.")
        sys.exit()
    ...
    if taxa_taxon_b_bold[taxon] > 1 or \
        ...
        print("")
        print(f"\nError in plot title taxon bold designation for {taxon} in "
              "Input file.")
        sys.exit()         
    ...
    if taxa_plot_rstyle[taxon] > 4 or \
       taxa_plot_rstyle[taxon] < 1 and taxon != "Zones":
        print("")
        print(f"\nError in plot taxon right spine style number designation "
              f"for {taxon} in Input file.")
        sys.exit()
    ...
    if taxa_plot_ls_colour[taxon] > col_max or \
        ...
        print("")
        print(f"\nError in plot taxon left spine colour number designation "
              f"for {taxon}.")
        sys.exit()       
    ...
    if taxa_plot_rs_colour[taxon] > col_max or \
        ...
        print("")
        print(f"\nError in plot taxon right spine colour number designation "
              f"for {taxon} in Input file.")
        sys.exit()
    ...
    if taxa_x_tick_maj_colour [taxon] > col_max or \
        ...
        print("")
        print(f"\nError in plot x major tick colour number designation for "
    ...
    if taxa_x_tick_min_colour [taxon] > col_max or \
        ...
        print("")
        print(f"\nError in plot x minor tick colour number designation for "
              f"{taxon} in Input file.")
        sys.exit() 

###############################################################################
###############################################################################
    # Create plot for first taxon to be plotted based on parameter choices if 
    # graph type is a barplot. Graph type 1.
...
###############################################################################
    # Create plot for first taxon to be plotted based on parameter choices if 
    # graph type is lineplot with depth bars. graph type 2. 
...
###############################################################################
    # Create plot for first taxon to be plotted based on parameter choices if 
    # graph type is a line plot Graph type 3.
...
###############################################################################
        ...
        if taxa_exag_type[taxon] == 4:
            if taxa_exag_col[taxon] not in range (1, col_max) \
                ...
                print("  ")                    
                print (f"\nExaggeration aesthetics out of bounds for {taxon} "
                       "in Input file.")
                sys.exit() 
...
###############################################################################
        ...
        if fill_trans_type[taxon] > 1 or \
            fill_trans_type[taxon] < 0 and taxon != "Zones":
            print("  ")
            print(f"\nError in fill colour transparency for {taxon} in Input "
                  "file.")
            sys.exit()

###############################################################################
###############################################################################
    # Create plot for taxon to be plotted based on parameter chouces if graph 
    # type is a line and marker plot. Graph type 5.
    
    elif taxon == data_list_1 and plot_type_1 == 5:
        
###############################################################################
        # Adjustments required if exaggeration has been specified to any plot.
        if exag["exag"].max() > 0: 
            if data_2[taxon].max() < 10:
                graph.set_ylim(0, yu_lim + 20)
            else:
...
###############################################################################
        ...
        if  line_colour_type[taxon] > col_max or \
            line_colour_type[taxon] < 1 and taxon != "Zones":
            print("  ")
            print(f"\nError in line colour number {taxon} in Input file.")
            sys.exit()
            
        if marker_typ_type[taxon] > 6 or \
            marker_typ_type[taxon] < 1  and taxon != "Zones":
            print("  ")
            print(f"\nError in marker type number for {taxon} in Input file.")
            sys.exit()
...
###############################################################################
###############################################################################
    # Create plot for taxon to be plotted based on parameter choices if graph 
    # type is a scatter plot. Graph type 6.
...
###############################################################################
    # Create plot for taxon to be plotted based on parameter choices. If graph
    # type is one of the two possible percentage graphs. Graph type 7.    
...
###############################################################################
        ...
        if num_perc == 1:
            perc_colours_1 = [colour(x) for x in perc_plot_1_colours]
            if perc_calc_1 == "yes":
                graph.stackplot(data_2["Depth"], perc_1_sums_1_calc, \
                                perc_1_sums_2_calc, perc_1_sums_3_calc, \
                                perc_1_sums_4_calc, perc_1_sums_5_calc, \
                                colors = perc_colours_1)
                
            graph.stackplot(data_2["Depth"], perc_1_sums_1, \
                            perc_1_sums_2, perc_1_sums_3, perc_1_sums_4, \
                            perc_1_sums_5, colors = perc_colours_1)
           
        if num_perc == 2:
            perc_colours_2 = [colour(x) for x in perc_plot_2_colours]
            if perc_calc_2 == "yes":
                graph.stackplot(data_2["Depth"], perc_2_sums_1_calc, \
                                perc_2_sums_2_calc, perc_2_sums_3_calc, \
                                perc_2_sums_4_calc, perc_2_sums_5_calc, \
                                colors = perc_colours_1)
            
            graph.stackplot(data_2["Depth"], perc_2_sums_1, \
                            perc_2_sums_2, perc_2_sums_3, \
                            perc_2_sums_4, perc_2_sums_5, \
                            colors = perc_colours_2)
...
###############################################################################
    ...
    # choices if graph type ia a bar plot. Graph type 1. 
...
###############################################################################
        ...
        if marker_e_col[taxon] > col_max or marker_e_col[taxon] < 1 \
            and taxon != "Zones":
            print("")
            print(f"\nError in marker type edge colour number for {taxon} in "
                  "Input file.")
            sys.exit()
...
###########################################################################
        ...
        if marker_typ_type[taxon] > 6 or marker_typ_type[taxon] < 1 \
            and taxon != "Zones":
            print(" ")
            print(f"\nError in arker type number for {taxon} in Input file.")
            sys.exit() 
            
        if marker_f_col[taxon] > col_max or marker_f_col[taxon] <1 \
            and taxon != "Zones":
            print("")
            print(f"\nError in marker type fill colour number for {taxon} in "
                  "Input file.")
            sys.exit()
...
###############################################################################
###############################################################################
# Percentage plots two are allowed at present but this can be expanded if 
# required. 
# Graph type 7
...
###############################################################################
        ...
        if num_perc == 1:
            if num_perc_plots == 1 or num_perc_plots == 2:
                perc_colours_1 = [colour(x) for x in perc_plot_1_colours]
                
                if perc_calc_1 == "yes":
                    perc_line_atts = []
                    perc_line_atts.append(perc_plot_1_line_colour)
                    perc_line_atts.append(perc_plot_1_lw)
                    
                    if any(i > 0 for i in perc_line_atts):
                        
                        perc_2 =[x + y for x, y in zip(perc_1_sums_1, \
                                                       perc_1_sums_2)]
                        perc_3 =[x + y + z for x, y, z in zip(perc_1_sums_1, \
                                                              perc_1_sums_2, \
                                                              perc_1_sums_3)]
                        perc_4 =[x + y + z + a for x, y, z, a \
                                 in zip(perc_1_sums_1, perc_1_sums_2, \
                                 perc_1_sums_3, perc_1_sums_4)]
                            
                        perc_5 =[x + y + z + a + b for x ,y ,z, a, b \
                                 in zip(perc_1_sums_1, perc_1_sums_2, \
                                 perc_1_sums_3, perc_1_sums_4, \
                                 perc_1_sums_5)]
                        ...
                        graph.plot(data_2["Depth"], perc_1_sums_1_calc, \
                                   linewidth = perc_plot_1_lw, \
                                   color = colour(perc_plot_1_line_colour))
                        graph.plot(data_2["Depth"], perc_1_sums_1_calc + \
                                   perc_1_sums_2_calc, \
                                   linewidth = perc_plot_1_lw, \
                                   color = colour(perc_plot_1_line_colour))
                ...
                if perc_calc_1 == "no" or pd.isnull(perc_calc_1) == True:
                    perc_line_atts = []
                    perc_line_atts.append(perc_plot_1_line_colour)
                    perc_line_atts.append(perc_plot_1_lw)
                    ...
                    if any(i > 0 for i in perc_line_atts):
                        ...
                        graph.stackplot(data_2["Depth"], perc_1_sums_1, \
                                        perc_1_sums_2, perc_1_sums_3, \
                                        perc_1_sums_4, perc_1_sums_5, \
                                        colors = perc_colours_1)
    
                        graph.plot(data_2["Depth"],perc_1_sums_1, \
                                   linewidth = perc_plot_1_lw, \
                                   color = colour(perc_plot_1_line_colour))
                            
                        graph.plot(data_2["Depth"], perc_2, \
                                   linewidth = perc_plot_1_lw, \
                                   color = colour(perc_plot_1_line_colour))
                            
                        graph.plot(data_2["Depth"], perc_3, \
                                   linewidth = perc_plot_1_lw, \
                                   color = colour(perc_plot_1_line_colour))
                            
                        graph.plot(data_2["Depth"], perc_4, \
                                   linewidth = perc_plot_1_lw, \
                                   color = colour(perc_plot_1_line_colour))
                        
                        graph.plot(data_2["Depth"], perc_5, \
                                   linewidth = perc_plot_1_lw, \
                                   color = colour(perc_plot_1_line_colour))
                    else:
                        graph.stackplot(data_2["Depth"],perc_1_sums_1, \
        ...
        if num_perc == 2:
            if num_perc_plots == 2:
                ...
                if perc_calc_2 == "yes":
                    perc_line_atts = []
                    perc_line_atts.append(perc_plot_2_line_colour)
                    perc_line_atts.append(perc_plot_2_lw)
                    
                    if any(i > 0 for i in perc_line_atts):
                        
                        perc_2 =[x + y for x, y in zip(perc_2_sums_1, \
                                                       perc_2_sums_2)]
                        perc_3 =[x + y + z for x, y, z in zip(perc_2_sums_1, \
                                                              perc_2_sums_2, \
                                                              perc_2_sums_3)]
                        perc_4 =[x + y + z + a for x, y, z, a \
                                 in zip(perc_2_sums_2, perc_2_sums_2, \
                                 perc_2_sums_3, perc_2_sums_4)]
                        ...
                        graph.plot(data_2["Depth"], perc_2_sums_1_calc, \
                                   linewidth = perc_plot_2_lw, \
                                   color = colour(perc_plot_2_line_colour))
                        graph.plot(data_2["Depth"], perc_2_sums_1_calc + \
                                   perc_2_sums_2_calc, \
                                   linewidth = perc_plot_2_lw, \
                                   color = colour(perc_plot_2_line_colour))
                ...
                if perc_calc_2 == "no" or pd.isnull(perc_calc_2) == True:
                    perc_line_atts = []
                    perc_line_atts.append(perc_plot_2_line_colour)
                    perc_line_atts.append(perc_plot_2_lw)
                    ...
                    if any(i > 0 for i in perc_line_atts):
                        ...
                        graph.plot(data_2["Depth"],perc_2_sums_1, \
                                   linewidth = perc_plot_2_lw, \
                                   color = colour(perc_plot_2_line_colour))
...
if title_text_on_off == "on":
    ...
    fig.text(title_y_pos, title_x_pos, title_text, \
...
if footer_text_on_off == "on":
    ...
    fig.text(footer_y_pos, footer_x_pos, footer_text, \
...
###############################################################################
###############################################################################
# Add grouping annotations. Up to 10 are allowed at present. Should be ample.
...
###############################################################################
###############################################################################
# Saves the overall plot to your selected drive stated in the parameter file. 
# Choose png, svg or pdf in the parameter file. Can choose to save as all
# types in one go.
print("")
...
if "png" in save_list:
    try:
        ...
        print("\nProblem with dpi entry in Parameter file. Check it is "
              "numeric and a sensible value. Values between 75 for a coarse "
              "resolution and 1000 for a high resolution image are most "
              "likley.")
        sys.exit()
                    
    if pd.isnull(dpi_num) == True:
        print("\nDpi entry required in Parameter file if png is specified as "
              "output format.")
        sys.exit()
